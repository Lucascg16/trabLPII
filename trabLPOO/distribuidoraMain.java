...
public class distribuidoraMain {
    ...
    static void excluirProduto() {
        ...
        int inicio = 0, fim, ultimo, primeiro, procurar = 0;
        boolean achou = false;
        iniciarArquivo();
        try {
            if (memoria.length() != 0) {
                procurar = (Integer.parseInt(JOptionPane.showInputDialog("Digite o odigo do produto que deseja excluir.")));
                produto = "";
                valor = "";
                quantidade = "";
                inicio = 0;
                ...
                JOptionPane.showMessageDialog(null, "Não há arquivos para serem excluidos.");
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(null, "Arquivo não encontrado");
        }
